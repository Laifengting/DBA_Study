...
sysbench_test_info_path=/opt/sysbench-test
function red_echo(){
        local what="$*"
...
function  purge_binlog(){
    port=$1
    mysql -u$user -p$passwd -P$port -h$host -e 'purge binary logs before now();'
}
...
function  sysbench_with_diff_thread(){
    ...
    order=$3
    test_mode=$4
    sysbench ${test_mode} --mysql_storage_engine=innodb  --table-size=10000 --tables=4 --mysql-db=sbtest --mysql-user=$user --mysql-password=$passwd --mysql-port=$port  --mysql-host=$host --threads=$thread_num  --time=60 --report-interval=2 --db-ps-mode=disable --events=0 --db-driver=mysql $order
}
function main(){
