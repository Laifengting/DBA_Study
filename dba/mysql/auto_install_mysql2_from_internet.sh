...
function check_install_mysql_environment() {
    ...
    if [ $(uname -s)="linux" ]; then
        ...
        echo "os is not linux,this is fail!"
        exit 1
    fi
...
function InstallMySQL() {
    ...
    if [ ${os_version}="el6" ]; then
        ...
        SERVERID=$(ifconfig ens33 | grep "inet " | awk '{ print $2}' | awk -F. '{ print $3$4}')
        ipaddr=$(ifconfig ens33 | grep "inet " | awk '{ print $2}' | awk -F. '{ print $3$4}')
    else
        SERVERID=100000
        ipaddr=100000
    fi
